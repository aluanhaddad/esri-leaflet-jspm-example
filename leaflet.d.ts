...
declare module "esri-leaflet" {
  import { FeatureGroup, ControlStatic} from "leaflet";
  import * as L from "leaflet";
  export interface ToMapAddable {
    addTo(map: L.Map): void;
  }

  export type Evented = { on: (eventId: string, handler: (event: L.GeoJSON) => void) => Evented }

  export function featureLayer(options: { url: string, style?: any }): L.GeoJSON & { setFeatureStyle(id, style: any); getFeature(id); };
  export function basemapLayer(url: string): ToMapAddable;
  export function get(url: string, params: { [key: string]: any }, callback: (error, response) => any);

  export namespace Util {
    export function template(template: string, values: { [key: string]: any });
  }
  export default { featureLayer, basemapLayer, get };
}
interface FeatureGroup<T> {
  on(type: string, fn: (e: L.LeafletEvent) => void, context?: any);
}
declare module EsriLeafletGeocoder {
  ...
  export function featureLayerProvider(options: {
    ...
    label: string;
    bufferRadius: number;
    formatSuggestion: (feature) => string;
  }): Provider;
  export function arcgisOnlineProvider(): Provider;
}

declare module "esri-leaflet-geocoder" {
  export = EsriLeafletGeocoder;
}
...
declare namespace L {
  ...
  export interface FeatureGroup<T extends ILayer> extends LayerGroup<T>, ILayer, IEventPowered<FeatureGroup<T>> {
    ...
    bindPopup(createPopup: (feature: FeatureGroup<GeoJSON> & PropertiesBearer & { feature: IdBearer & PropertiesBearer }) => string): FeatureGroup<T>;
    ...
    on(type: string, fn: (e: LeafletGeoJSONEvent & FeatureGroup<TileLayer> & {
      feature: TileLayer & PropertiesBearer & IdBearer;
      layer: { feature: TileLayer & PropertiesBearer & IdBearer; bringToFront(); }
    }) => void, context?: any): FeatureGroup<T>;
...
declare namespace L {
  ...
  * of the iterator function.
    ...
    on(type: string, fn: (e: LeafletGeoJSONEvent) => void, context?: any): Map;
